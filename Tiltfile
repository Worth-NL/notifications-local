...
if config.tilt_subcommand == 'up':
  for repo in repo_list:
    if not os.path.exists(path='../{}'.format(repo)):
      git_checkout(repository_url='{}{}'.format(repo_base, repo), checkout_dir='../{}'.format(repo), quiet=True)
    
    local(command='git fetch', dir='../{}'.format(repo), quiet=True)
    
    if repo in merge_list:
      print('Checking out merge branch for {}'.format(repo))
      local(command='git checkout alphagovMerge', dir='../{}'.format(repo), quiet=True, echo_off=True)

    if repo == 'notifications-utils':
      local(command='git checkout alphagovMerge9401', dir='../{}'.format(repo), quiet=True, echo_off=True)
  ...
  ## Admin
  local(command='npm install', dir='../notifications-admin', env={'NOTIFY_ENVIRONMENT': 'development'}, quiet=True, echo_off=True)
  local(command='npm run build', dir='../notifications-admin', env={'NOTIFY_ENVIRONMENT': 'development'}, quiet=True, echo_off=True)
  ...
  local(command='./scripts/venv_wrapper.sh uv pip uninstall govuk-frontend-jinja', quiet=True, echo_off=True)
  local(command='./scripts/venv_wrapper.sh uv pip install -e ../LandRegistry-frontend-jinja --config-settings editable_mode=compat', quiet=True, echo_off=True)
...
if any([keyword in profiles for keyword in ['stub', 'full']]):
